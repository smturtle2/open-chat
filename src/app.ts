...
import { sessionRoot, sessionMode, uploadsAbsDir, uploadsRelDir, deleteChatWorkspace, workspaceState } from "./agent/sessionPaths.js";
import {
  createProvider,
  deleteProvider,
  getSettings,
...
app.get("/api/sessions", (c) => {
  const sessions = db.listSessions().map((s) => ({ ...s, workspace_state: workspaceState(s) }));
...
app.get("/api/providers", (c) => {
  return c.json(listProviders().map(publicProvider));
});
...
app.get("/api/sessions/:id", (c) => {
  ...
  const run = db.getRunSnapshot(id);
  db.touchSession(id);
  return c.json({ ...session, status: isRunning ? "running" : session.status, messages, run, workspace_state: workspaceState(session), last_event_id: lastEventId });
});
...
app.delete("/api/sessions/:id", async (c) => {
  const id = c.req.param("id");
  if (!db.getSession(id)) return c.json({ error: "Session not found" }, 404);
...
app.get("/api/sessions/:id/files", (c) => {
  ...
  const session = db.getSession(id);
  if (!session) return c.json({ error: "Session not found" }, 404);
  const rootDir = sessionRoot(session);
  db.touchSession(id);
...
app.post("/api/sessions/:id/messages", async (c) => {
  ...
  let prompt = typeof body.content === "string" ? body.content.trim() : "";
  ...
  const runId = coordinator.submit(id, prompt, attachmentIds, clientMsgId);
  return c.json({ status: "submitted", run_id: runId });
});
...
app.post("/api/sessions/:id/messages/:messageId/edit", async (c) => {
  ...
  if (!db.getMessages(id).some((m) => m.id === messageId && m.role === "user")) return c.json({ error: "Target user message not found" }, 404);

  // Coordinator waits for the old loop to drain before mutating history.
  const runId = coordinator.regenerateFrom(id, messageId, newContent);
  return c.json({ status: "submitted", run_id: runId });
});
...
app.post("/api/sessions/:id/messages/:messageId/regenerate", async (c) => {
  ...
  const runId = coordinator.regenerateFrom(id, messageId, null);
  return c.json({ status: "submitted", run_id: runId });
});
